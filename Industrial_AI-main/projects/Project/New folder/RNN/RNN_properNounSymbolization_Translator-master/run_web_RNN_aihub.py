...
PN_dict = ""
PN_list = ""
PN_dict_name = 'aihub/PN_version/vocab/extractPN.pkl'
lang = "k2e"
...
def k2e_trans(num=None):
    ...
    if request.method == 'POST':
        ...
        text_file = open("input.txt.tok", "r", encoding="utf8")
        ...
        replaced_sen, info_dict = convert_pn_for_web(replaced_sen, PN_list, info_dict, lang)
        print("replaced_sen : " + replaced_sen)
        print("info_dict : ", info_dict)
        text_file.close()


        text_file = open("input.txt.tok.pn", "w", encoding="utf8")
        ...
        apply_bpe=check_output("../subword_nmt/apply_bpe.py -c " +\
                                "./aihub/PN_version/subword/kr.10000.code " +\
                                "< ./input.txt.tok.pn > ./input.txt.tok.pn.sub", shell=True)
...
def add_dict(num=None):
    ...
    if request.method == 'POST':
        ...
        global PN_list
        word_tuple_kr = (kr_word, en_word)
        word_tuple_en = (en_word, kr_word)
        PN_list.append(word_tuple_kr)
        PN_list.append(word_tuple_en)
        # info_dict[kr_word] = en_word
        # info_dict[en_word] = kr_word
        print(1)
        print(PN_list)

        admin_word_kr = "aihub/PN_version/admin_word_kr.txt"
        with open(admin_word_kr, 'a') as f:
            f.write(kr_word+"\n")

        admin_word_en = "aihub/PN_version/admin_word_en.txt"
        with open(admin_word_en, 'a') as f:
            f.write(en_word+"\n")
